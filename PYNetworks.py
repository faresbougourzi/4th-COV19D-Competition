...
# -*- coding: utf-8 -*-
"""
Created on Mon Jan 10 11:09:17 2022
...
class YUNet(nn.Module):
    ...
    def forward(self, input):
        ...
        x3_1 = self.conv3_1(torch.cat([x3_0, self.up(x4_0)], 1))
        x3_1_2 = self.conv3_1(torch.cat([x3_0, self.up(x4_0)], 1))
        x2_2 = self.conv2_2(torch.cat([x2_0, self.up(x3_1)], 1))
        x2_2_2 = self.conv2_2(torch.cat([x2_0, self.up(x3_1_2)], 1))
        x1_3 = self.conv1_3(torch.cat([x1_0, self.up(x2_2)], 1))
        x1_3_2 = self.conv1_3(torch.cat([x1_0, self.up(x2_2_2)], 1))
        x0_4 = self.conv0_4(torch.cat([x0_0, self.up(x1_3)], 1))
        x0_4_2 = self.conv0_4(torch.cat([x0_0, self.up(x1_3_2)], 1))
...
#### YUnetplus #########################################################
class YUNetplus(nn.Module):
    def __init__(self, input_channels=3, num_classes = 1, deep_supervision=False):
        super(YUNetplus, self).__init__()
        ...
        self.conv1_2 = DoubleConv(nb_filter[1]*2+nb_filter[2], nb_filter[1])
        self.conv2_2 = DoubleConv(nb_filter[2]*2+nb_filter[3], nb_filter[2])
        self.conv2_2_2 = DoubleConv(nb_filter[2]*2+nb_filter[3], nb_filter[2])

        self.conv0_3 = DoubleConv(nb_filter[0]*3+nb_filter[1], nb_filter[0])
        self.conv1_3 = DoubleConv(nb_filter[1]*3+nb_filter[2], nb_filter[1])
        self.conv1_3_2 = DoubleConv(nb_filter[1]*3+nb_filter[2], nb_filter[1])

        self.conv0_4 = DoubleConv(nb_filter[0]*4+nb_filter[1], nb_filter[0])
        self.conv0_4_2 = DoubleConv(nb_filter[0]*4+nb_filter[1], nb_filter[0])
    ...
    def forward(self, input):
        ...
        x3_1_2 = self.conv3_1(torch.cat([x3_0, self.up(x4_0)], 1))
        x2_2 = self.conv2_2(torch.cat([x2_0, x2_1, self.up(x3_1)], 1))
        x2_2_2 = self.conv2_2(torch.cat([x2_0, x2_1, self.up(x3_1_2)], 1))
        x1_3 = self.conv1_3(torch.cat([x1_0, x1_1, x1_2, self.up(x2_2)], 1))
        x1_3_2 = self.conv1_3(torch.cat([x1_0, x1_1, x1_2, self.up(x2_2_2)], 1))
        x0_4 = self.conv0_4(torch.cat([x0_0, x0_1, x0_2, x0_3, self.up(x1_3)], 1))
        x0_4_2 = self.conv0_4(torch.cat([x0_0, x0_1, x0_2, x0_3, self.up(x1_3_2)], 1))
...
#### AttUNet 2 Encoders #########################################################
class Att2EUNet(nn.Module):
    def __init__(self, input_channels=3, num_classes = 1, deep_supervision=False):
        super(Att2EUNet, self).__init__()
        ...
        self.conv01_0 = DoubleConv(input_channels, nb_filter[0])
        self.conv11_0 = DoubleConv(nb_filter[0], nb_filter[1])
        self.conv21_0 = DoubleConv(nb_filter[1], nb_filter[2])
        self.conv31_0 = DoubleConv(nb_filter[2], nb_filter[3])
        # self.conv41_0 = DoubleConv(nb_filter[3], nb_filter[4])
    ...
    def forward(self, input1, input2):
        # Images
        # image_size = input1.shape
      
        # encoding path
        # 1
        x0_0 = self.conv0_0(input1)        
        x11_0 = self.conv01_0(input2)
        
        x1_0att = self.Attdw1(g=x11_0, x=x0_0)
        
        x1_0 = self.conv1_0(self.pool(x1_0att))
        x12_0 = self.conv11_0(self.pool(x11_0))
        
        #2 
        x2_0att = self.Attdw2(g=x12_0, x=x1_0)
        
        x2_0 = self.conv2_0(self.pool(x2_0att))
        x13_0 = self.conv21_0(self.pool(x12_0))
        
        #3 
        x3_0att = self.Attdw3(g=x13_0, x=x2_0)
        
        x3_0 = self.conv3_0(self.pool(x3_0att))
        x14_0 = self.conv31_0(self.pool(x13_0))
       
        #4 
        x4_0att = self.Attdw4(g=x14_0, x=x3_0)
        
        x4_0 = self.conv4_0(self.pool(x4_0att))
        # print(x4_0.shape)
        # x12_0 = self.conv41_0(x11_0)                             
...
#### YAttUNet #########################################################
class YAttUNet(nn.Module):
    def __init__(self, input_channels=3, num_classes = 1, deep_supervision=False):
        super(YAttUNet, self).__init__()
